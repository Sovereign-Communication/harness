...
9. **`DF-LING-1`** — PR #76 open; implementation + full gates reported green, independent Sonnet review was the pending tenth workflow step as of the latest Claude session record, and merge CI still pending. After landing, `DF-LING-2` remains an operator decision; do not change the free apply pool without evidence and ruling.
10. **HUL-D follow-ups** — `DF-HUL-1..3`; existing `fix/hul-d-followups` worktree has uncommitted changes across CLI, mission driver, docs, and tests. Finish in its own PR; do not mark complete from the worktree state.
11. **`JEV-AUDIT-GATE`** — preserved branch is 6 commits ahead and 1 behind `origin/main`, plus a local CHANGELOG modification; this was not part of the active Claude serial workflow. Rebase, inspect the partial-run and keyed-path behavior, run named gates, and land in its own PR.
12. **`HV-0` JEV integration foundation + vision-assessment pilot** — first Hourglass vision slice; execute the assessment only after the prompt/pack has full Claude plan context and a preflighted single-call budget. Use the JEV category output to validate/refine the vision before opening three parallel lanes.
13. **`HV-1`, `HV-2`, `HV-3`** — after `HV-0`, launch the three disjoint-owner lanes in separate worktrees: stage-specific JEV integrations; evidence-bearing context brief; token allowance/accounting owner. See the detailed `HV-*` table above and paste-ready prompts in [hourglass-freebuff-prompts.md](hourglass-freebuff-prompts.md).
...
15. **Remaining existing roadmap work** — `DF-MS-2b`; HUL-D driver follow-ups above; UI parity `DF-UI-1..3`; `JEV-P4` dogfood A/B + freeze persistence; `JEV-P6` stage C (`waist-brief`, `noul-audit`, freshness); operator rulings such as `DF-LING-2`; then Exit rows. Reconcile duplicates/closed items as each PR lands; do not infer they are complete from the vision plan.
...
**Priority and parallelization rule:** do `HV-0` first. It builds the reusable JEV integration contract plus the vision-assessment vertical slice, then uses the resulting assessment as the first Freebuff-lane pilot. Review its declared categories and improve any non-perfect categories before declaring the pilot useful. Only after this JEV path works should three implementation lanes (`HV-1`, `HV-2`, `HV-3`) launch concurrently in separate worktrees. Their primary owners are disjoint. Integrators must not edit another lane's owned files; propose interface changes back to the kickoff owner. Merge one PR at a time after rebase and green gates. `HV-4` depends on those contracts and merges; `HV-5` depends on `HV-4`; `HV-6` closes parity and measured acceptance. Each is an independently reviewable PR.
...
|---|---|---|---|
| `HV-0` JEV integration foundation + vision-assessment pilot | Establish a composable, named JEV integration contract through one real vertical slice: assess the Hourglass vision using declared categories (modularity/composability, token-budget shape, grounding, planning quality, execution boundary, JEV integration coverage, sovereignty, observability, verification, cost/resource bounds). Return per-category level/score, evidence, and declared improvement buckets; a perfect assessment means every category is at its top declared level and no improvement bucket remains. Keep this separate from `jev-phase`, implementation readiness, or a code-completion gate. Prove keyed behavior hermetically and report fallback honestly. If live key is available, run a separately preflighted assessment, record its model/cost/evidence, then use its category report as the first Freebuff-lane pilot; improve the vision where the report identifies a real gap. If no key, record that live pilot is blocked and do not substitute heuristic output for JEV. | `harness/jev_policy.py`, `harness/jev_packs.py`, design assessment pack and `tests/test_jev_vision_assessment.py`; one CLI/artifact entry point only if a current surface can own it cleanly. | First, before parallel lanes. Owns and freezes the shared JEV integration contract used by later integrations. No arbitrary executable plugin loader. |
| `HV-1` stage-specific JEV integration coverage | Apply the HV-0 contract to the distinct Hourglass judgment points. Provide selectable typed integrations for context relevance/coverage/conflicts, plan sufficiency/soundness/evidence requests, execution suitability/checkpoints, consent freshness/defer/escalation signals, and verification claims. For each, name code-owned facts versus JEV semantic judgment, pack/version, abstention/fallback, and evidence; avoid redundant calls when no decision is needed. | `harness/jev_policy.py`, `harness/jev_packs.py`, JEV integration matrix and `tests/test_hourglass_jev_integrations.py`. Do not edit `condenser.py`, spend/config budget owner, waist composition, CLI/MCP surfaces; publish APIs. | One of three parallel lanes after HV-0. Extends the new integration protocol; no second Jev client/policy. |
...
| `DF-CLI-2` | med | `harness verify --panel` is declared in the parser but never forwarded (silently ignored) | `panel` / CLI | forward to `service.run_verify` | `tests/test_cli.py` (in `fix/verify-panel-hg-preview`) |
| `DF-BOD-1` | med | a 5-model panel with `max_panelists=5` seats only 4 (`google/gemini-3.8-flash` silently dropped, `required_panelists=4`) — the Board's 5/5 rule is unreachable | `panel` | seat N of N eligible or report the drop in the envelope | panel tests (in `fix/verify-panel-hg-preview`) |
