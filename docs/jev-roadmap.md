...
10. **`JEV-AUDIT-GATE`** — first open implementation item; preserved branch is unmerged and has a local CHANGELOG modification. Rebase, inspect the partial-run and keyed-path behavior, run named gates, and land in its own PR. It was outside Claude's completed serial workflow.
11. **HUL-D follow-ups** — `DF-HUL-1..3`; existing `fix/hul-d-followups` worktree has uncommitted changes across CLI, mission driver, docs, and tests. Finish in its own PR; do not mark complete from the worktree state.
