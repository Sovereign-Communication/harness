...
| OC-HANDOFF findings-only lane | open / gated | PR #73 remains an experimental reference and is not accepted as a worker. The fixed local worker is in `examples/oc_handoff/worker.py`; it accepts signed findings, reviews a bounded semantic summary, and can write only HANDOFF/OC_FINDINGS.md in its executing Harness checkout. No external runtime/config changes are in scope; never target seat-owned HANDOFF/*_STATE.md files. See the gate contract below. |
...
This Harness-owned OC handoff task is in scope but remains gated. PR #73 remains experimental and is not accepted as a worker. The local implementation in `examples/oc_handoff/worker.py` pins execution to its Harness checkout, requires explicit local signing approval, bounds the Jev request, and limits repository output to HANDOFF/OC_FINDINGS.md. Never infer or target seat-owned HANDOFF/*_STATE.md files. All implementation, tests, and documentation changes stay in this repository; no external runtime/config changes are included. PR #89 was documentation-only.
...
| OC findings handoff | OC-HANDOFF | `examples/oc_handoff/worker.py`; root pinned to its executing checkout; output HANDOFF/OC_FINDINGS.md | `tests/test_oc_handoff_worker.py` (signed approval, expiry/replay, fixed output, path containment, bounded Jev request/spend, recovery/outbox); exact-file diff audit | open / gated — implementation exists in Harness; PR #73 remains experimental; required CI and Jev gates remain pending |
