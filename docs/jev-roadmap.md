...
| OC-HANDOFF findings-only lane | open / gated | Safe local worker PR #90 MERGED `6aea14b`; CI run 296 passed all five jobs. PR #73 is closed as superseded. The worker in `examples/oc_handoff/worker.py` accepts signed findings, reviews a bounded semantic summary, and can write only HANDOFF/OC_FINDINGS.md in its executing Harness checkout. Its Jev intake on an actual signed, locally approved candidate and resulting receipt/output audit remain pending; `harness jev-phase --phase OC-HANDOFF` has not been run. This checkout contains no candidate or HANDOFF/OC_FINDINGS.md. No external runtime/config changes are in scope; never target seat-owned HANDOFF/*_STATE.md files. See the gate contract below. |
...
Worker enforcement, hermetic tests, independent review, and CI are complete in PR #90. Keep OC-HANDOFF open / gated until an actual signed, locally approved findings candidate completes the native Jev check, produces a receipt, and its output passes the exact-file diff audit; then `harness jev-phase --phase OC-HANDOFF` must pass before the lane is marked complete. The current checkout has no such candidate or findings file. This does not displace HV-0 as the first canon implementation slice.
...
| OC findings handoff | OC-HANDOFF | `examples/oc_handoff/worker.py`; root pinned to its executing checkout; output HANDOFF/OC_FINDINGS.md | `tests/test_oc_handoff_worker.py` (signed approval, expiry/replay, fixed output, path containment, bounded Jev request/spend, recovery/outbox); exact-file diff audit | open / gated — PR #90 merged `6aea14b`; focused tests, independent review, and CI run 296 passed; PR #73 closed as superseded; real signed-candidate Jev intake, receipt, and output audit remain pending because no candidate is present; `harness jev-phase --phase OC-HANDOFF` has not been run |
