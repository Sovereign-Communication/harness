...
### P0 — contract defects
1. **Pricing inaccurate (historical finding).** TypeSafe `usage` is `{input_tokens, output_tokens}` only; the original client used an incorrect **$42/Mtok** input rate and did not invent a provider `usage.cost`. HV-0 applies the operator-verified account rate of **$0.0042/Mtok** through the shared helper; output remains free.
...
| ID | Work |
|---|---|
| `JEV-P0-cost` | Historical P0 math used `input_tokens * 42 / 1e6`; HV-0 supersedes that rate with the operator-verified `$0.0042/Mtok` account rate. Never invent `usage.cost`; expose observed tokens on `JevEvaluationResult`. |
...
The original wf_1c82997e-f19 lane inventory has 23 items: 19 superseded and four non-superseded. Of those four, OC-HANDOFF remains in scope and gated; the safe local worker landed in PR #90 (6aea14b0d4aefd64a59190ad16378040775f3fbd), while PR #73 is closed as superseded. operator_tree_main has only its media portion integrated by PR #87 (8631ecad17d72ec62b1fae5f26ed31ed9fd5c210); worktree_ui_faces / DF-UI-2 remains open; and PR #88 (65da7b13dbe546238268bd986457b0dc96af4766) partially integrates the answer-lifecycle portion of worktree_jev_hourglass. PR #86 Ling rotation (a2cbb21859dcc7c95f1bc8deef168ccb246cede2) is separate from this inventory. PR #88 is only a partial HV-1 slice; no HV row is complete, and HV-0 is in progress.
...
| `HV-0` JEV integration foundation + vision-assessment pilot | **in progress** | scoped implementation and validation in `codex/hv-0-jev-vision-assessment`; phase contract and hermetic gates added; live pilot and PR merge gates pending |
...
| OC-HANDOFF findings-only lane | open / gated | Safe local worker PR #90 MERGED `6aea14b`; CI run 296 passed all five jobs. PR #73 is closed as superseded. The worker in `examples/oc_handoff/worker.py` accepts signed findings, reviews a bounded semantic summary, and can write only HANDOFF/OC_FINDINGS.md in its executing Harness checkout. A 2026-09-24 local-only phase preflight exited 1 at score 84.99 and correctly failed the required-output and verified-receipt gates. The isolated local self-audit passed 10.00/10 in A/R/SM/SD with R13, Ruff, and coverage green; its Jev judgment used the documented offline fallback, and a live judgment remains pending. The actual signed-candidate Jev intake and receipt/output audit remain pending because this checkout has no candidate or findings file. No external runtime/config changes are in scope; never target seat-owned HANDOFF/*_STATE.md files. See the gate contract below. |
...
This Harness-owned OC handoff task is in scope but remains gated. PR #73 is closed as superseded; the safe local implementation in `examples/oc_handoff/worker.py` landed in PR #90 (`6aea14b`) after independent review, focused tests, and green CI run 296. It pins execution to its Harness checkout, requires explicit local signing approval, bounds the Jev request, and limits repository output to HANDOFF/OC_FINDINGS.md. A 2026-09-24 `--local-only` phase preflight exited 1 at score 84.99, as expected with no output or verified receipt. The isolated local self-audit passed 10.00/10 in all four dimensions, with R13, Ruff, and changed-line coverage green; Jev used its offline fallback, so that result is not a live semantic judgment. The actual signed-candidate Jev intake and receipt/output audit remain pending because this checkout contains no candidate or findings file. Never infer or target seat-owned HANDOFF/*_STATE.md files. All implementation, tests, and documentation changes stay in this repository; no external runtime/config changes are included. PR #89 was documentation-only.

The merged worker and its hermetic suite implement the following controls:

- The worker pins its root to the executing Harness checkout; it does not accept a repository root from a manifest. It uses Harness WorktreeIsolation, allows only HANDOFF/OC_FINDINGS.md, rejects manifest-supplied path changes, and audits the exact changed-file set before merge.
- It requires locally authenticated identity and explicit operator approval for signed manifests, and validates signatures, expiry, and replay protection before work starts.
...
- Crash/restart recovery reconciles database, lock, and outbox state without duplicate, lost, or unverified handoff writes.
- Hermetic tests cover fixed root/path allowlisting; manifest authentication, approval, expiry, and replay rejection; fixed state roots and verifier; rejection of manifest-controlled executables; exact-file diff auditing; egress/spend policy; and crash/restart/outbox reconciliation.

Worker enforcement, hermetic tests, independent review, and CI are complete in PR #90. The phase bar now checks required-file presence independently of overridable `--evidence` blockers. Its OC-specific hard gate reads the archived signed manifest and local worker receipt/outbox, verifies both the operator attestation and keyed receipt HMAC with `HARNESS_OC_HANDOFF_HMAC_KEY`, checks the authenticated Jev acceptance threshold and non-fallback native pass within the $0.05 / 1,024-token bounds, bounds persisted file/state reads, validates the receipt and manifest digests, and rechecks that the receipt commit changes only `HANDOFF/OC_FINDINGS.md`, has the recorded base as its parent, is merged into `HEAD`, and matches the current output hash. A handoff file by itself cannot pass the phase bar. Keep OC-HANDOFF open / gated until an actual signed, locally approved findings candidate completes that path and `harness jev-phase --phase OC-HANDOFF` passes. The current checkout has no such candidate, receipt, or findings file. HV-0 is the first canon implementation slice now in progress.
...
| OC findings handoff | OC-HANDOFF | `examples/oc_handoff/worker.py`; root pinned to its executing checkout; output HANDOFF/OC_FINDINGS.md | `tests/test_oc_handoff_worker.py` (signed approval, expiry/replay, fixed output, path containment, bounded Jev request/spend, recovery/outbox); exact-file diff audit | open / gated — PR #90 merged `6aea14b`; focused tests, independent review, and CI run 296 passed; PR #73 closed as superseded; 2026-09-24 local-only preflight correctly blocked at score 84.99; real signed-candidate Jev intake, receipt, and output audit remain pending because no candidate is present |
...
9. ~~**`DF-LING-1`**~~ — **complete** (PR #76 merged `c5c4180`; ~~`DF-LING-2`~~ **complete**, operator ruling 2026-09-23: Ling in rotation, never default).
...
**`HV-1` progress (PR #88, integrated from its source branch):** delivers one slice of `HV-1`'s "final completeness/alignment" and "plan sufficiency" judgment points as a bounded answer/reiterate loop, folded into the existing `JevPolicy`/`jev_packs` owner (no second contract, no second Jev client):
- `harness/jev_packs.py` adds `answer_question_pack`/`ANSWER_PACK_VERSION` (`answer-sufficiency-v1`): typed `answer_sufficient` / `iteration_required` / `plan_required` nouls.
- `harness/jev_policy.py` adds `JevPolicy.evaluate_answer(prompt, answer, context, ...)`: one reservation/settlement/ledger event per call, honest `native`/fallback/malformed distinction, never promotes a fallback to a passing signal.
- `harness/orchestrator.py`'s `drive()` gains an opt-in `jev_completion_threshold` that reads the existing completion-nouls `goal_achieved` signal (via a new `_result_probability` helper) and requires another round before the generative judge spends when the live Jev support is below the caller's threshold; existing callers that omit the threshold are unaffected.
- `harness/agent.py` adds `AutonomousAgent.run_hourglass_request(...)`: for a direct-answer request it runs bounded local context intake, the existing panel/judge/escalation ladder, and `evaluate_answer` in a capped reiterate loop (`ok` / `deferred` / `plan_required` / `needs_iteration`); a `plan_required` Jev signal is advisory only and never crosses the write boundary — only `_handle_edit` (now with `use_jev_completion=True`, wiring the same threshold into `drive()`) can write. `tests/test_jev_answer_lifecycle.py` covers both paths hermetically (keyed/unkeyed/malformed/transport-failure `evaluate_answer`; ladder rotation/failure/truncation; the reiterate loop including budget-deferred and plan-required exits; the edit lane's `use_jev_completion` wiring).
- Not yet delivered from `HV-1`'s full contract: context-intake relevance/coverage/conflict judgments, execution-checkpoint and consent-freshness signals, and the declared context/planning/execution **restart-target enum** (this slice only distinguishes `deferred`/`plan_required`/`needs_iteration`/`ok`, it does not select a stage to restart). Those remain open under `HV-1`.
- **Left out of PR #88:** the source branch also carried (a) the shared Jev price correction, which belongs to HV-0 and is implemented in the HV-0 slice, and (b) a dogfood coordinator module without a canon row. That module remains excluded unless an operational canon row authorizes it; it is not part of HV-1 answer lifecycle.
...
| `DF-LING-2` | med | PR #69 removed Ling from `FREE_APPLY_POOL` on an unevidenced capability claim, without the operator sign-off its own plan required | `MS-*` | **fixed** (operator ruling 2026-09-23: Ling may be in rotation but never the default): `FREE_APPLY_POOL` restores `inclusionai/ling-3.0-flash-fin:free` at a non-first position; `resolve_tier_recommended_model` / `tier_model_ladder` / default apply resolution still never return Ling first | `tests/test_sliding_scale.py::test_ling_in_rotation_but_never_default` |
