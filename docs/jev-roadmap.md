...
11. **HUL-D follow-ups** — `DF-HUL-1..3`; PR #78 (`fix/hul-d-followups`) is open, not yet merged. Finish in its own PR; do not mark complete from the worktree state.
...
| `DF-MS-2b` | med | verify preflight still reserves every configured retry (~90x the actual spend) — PR #69 took the documentation branch only | `MS-context` / `panel` | **fix opened** (branch `fix/ms-2b-verify-preflight`): `panel._judge_fallback_reserve` now sizes the judge-fallback reserve to `panel_pool[max_panelists:]` -- the dispatch tail that can actually go undispatched -- instead of the whole pool, since every panelist within `max_panelists` is guaranteed to be attempted as a vote and can never also be an untried judge-fallback candidate; `--max-cost` help text updated off the stale `$0.036` figure. Pending CI/merge before marking complete | `tests/test_panel.py` |
...
| `DF-LEDGER-1` | high | `AutonomyLedger._rotated_paths` treated ANY `ledger.jsonl.*` file as a chain segment (matched only the shared prefix), so a human backup (`ledger.jsonl.bak-20260918-dogfood`) was loaded as evidence, its lines quarantined wholesale, and the chain reported broken | `harness/ledger.py` | **fixed in this PR**: segment discovery matches only the exact rotator suffix format (`%Y%m%dT%H%M%S.%f`); non-matching files are skipped with a one-line stderr note | `tests/test_ledger.py::LedgerSegmentDiscoveryTests` |
| `DF-AUDIT-2` | med | `harness/worktree.py` `WorktreeIsolation.create` named branches/paths deterministically from the node id alone (`harness/task_N`); concurrent runs against the same repo (or a crashed run's leftover) collide on that fixed name and block later runs (observed blocking R13) | `harness/worktree.py` | **fixed in this PR**: branch/path name carries a per-call unique suffix (pid + short uuid); cleanup already runs in `executor.py`'s `finally` | `tests/test_worktree.py::WorktreeIsolationTests::test_same_node_id_gets_unique_branch_and_path`, `test_leftover_branch_never_blocks_a_later_create` |
