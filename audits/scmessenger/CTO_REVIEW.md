...
**Independent confirmation (pass 3) — the three load-bearing claims were re-verified with methods other than the original filesystem grep, all confirmed:**
- **(a) Zero non-test call sites** — re-run with index-based `git grep` over the whole tracked tree (all languages): `verify_bundle` callers are only `core/src/identity/keys.rs` tests + the `mod.rs` re-export; `save_contact_bundle` callers are only `contacts.rs:1107` (unit test) and three integration tests. The SCMessenger project's own `HANDOFF/CTO_STATE.md:549–551` states the same conclusion independently ("`verify_bundle` … has ZERO production callers, and `save_contact_bundle` performs no signature check before persisting. Latent today because `save_contact_bundle` itself has no production caller."). `git log -S` across **all branches**: `verify_bundle(` never appears in `iron_core.rs`/`mobile_bridge.rs` on any branch; `save_contact_bundle`'s usage count never changed since it was introduced (`85a19fed`, PQC-03) — no caller was ever added or removed.
- **(b) `get_contact_bundle` returns `None` on the live paths** — only call sites in the tree are `iron_core.rs:977` (send) and `:3477` (receive); no production writer exists: `contact_bundle_key` has no other users (nothing bypasses `save_contact_bundle` with raw store puts), `ContactStore` is referenced by no bridge/cloud/app layer, and the only `PublicKeyBundle` references outside `core/` are dev scratch scripts (`scripts/p2_06_*.py`). Git history: the *read* side was wired into send/receive in `6059038c` and no branch ever wired the *write* side.
- **(c) FFI exposes no bundle API** — three independent binding surfaces, all empty of bundle APIs: `core/src/api.udl` (0 bundle mentions), the generated `iOS/.../Generated/api.swift` (135 exported functions, 0 bundle mentions; the exposed `ContactManager` protocol's 17 methods — add/get/list/markVerified/mergeRemoteContacts/nickname/deviceId/lastSeen — take no bundle), and `mobile_bridge.rs`'s identity exports (own-identity backup import/export only). Desktop bridge UDLs likewise contain no bundle API.
- **Residual caveat, stated honestly:** this certifies *this workspace as committed*. If a future backend or app layer starts storing peer bundles outside these paths (e.g., a new FFI method or a server-side sync writing the contact store directly), the latent findings in §3A/§3E become live — which is exactly why the review order says to decide the wiring policy before shipping the hybrid path.
