...
- **UI + MCP read faces (`DF-UI-2`)** — thin, read-mostly faces over existing owners: `GET /api/jev-phase` (optional `?phase=`, always local-only over `harness.jev_completion`, same as the new MCP `jev_phase` tool), `GET /api/cost` (over `AutonomyLedger.cost_report`, same as `harness cost`), and `GET /api/missions` / `GET /api/missions/<id>` (over `harness.mission_record`, same as `mission_status`) in `harness/server.py`; the MCP server gains a `jev_phase` tool in `harness/mcp.py`/`harness/mcp_schemas.py`. `plan` stays CLI/MCP-only, documented in `docs/ui-readiness.md` (no server runner kind fits the planner's confirm/waist flow). Tests in `tests/test_server.py`/`tests/test_mcp.py`; README MCP tool list updated.
