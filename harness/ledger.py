...
class AutonomyLedger(LedgerAnalytics):
    """Append-only, hash-chained JSONL evidence ledger.

    Storage/integrity lifecycle lives here (append, hash chain, rotation,
    repair); the read-only analytics (participation_report) are mixed in
    from :mod:`harness.ledger_analytics` so all call sites keep their
    shape. One owner per concern: this module writes and proves the
    record; ledger_analytics reads it.
    """
